...
static request_t reqs(int fd, int size) {
  request_t r = {.fd = fd, .filesize = size};
  return r;
}
...
/* Test F — SFF with sizes (single thread; no workers): */
static void test_sff_picks_smallest(void) {
    request_t r;
    buffer_init(4, SCHED_SFF);

    buffer_produce(reqs(10, 1000)); /* big */
    buffer_produce(reqs(11, 10));   /* small */
    buffer_produce(reqs(12, 500));  /* medium */

    buffer_consume(&r); assert(r.fd == 11); /* 10 bytes */
    buffer_consume(&r); assert(r.fd == 12); /* 500 */
    buffer_consume(&r); assert(r.fd == 10); /* 1000 */

    buffer_destroy();
    printf("Test F OK (SFF smallest first)\n");
}

/* Test G — SFF with all -1 still FIFO: */
static void test_sff_unknown_is_fifo(void) {
    request_t r;
    buffer_init(3, SCHED_SFF);
    buffer_produce(req(1));
    buffer_produce(req(2));
    buffer_produce(req(3));
    buffer_consume(&r); assert(r.fd == 1);
    buffer_consume(&r); assert(r.fd == 2);
    buffer_consume(&r); assert(r.fd == 3);
    buffer_destroy();
    printf("Test G OK (SFF with -1 == FIFO)\n");
}
