...
int main(int argc, char *argv[]) {
    ...
    while ((c = getopt(argc, argv, "d:p:t:b:s:")) != -1) {
        ...
        case 's':
            ...
            break;
        default:
            fprintf(stderr, "usage: wserver [-d basedir] [-p port] [-t threads] [-b buffers] [-s FIFO|SFF]\n");
            exit(1);
        }
    }
    ...
    /* Phase 3: SFF not implemented yet — force FIFO behavior */
    if (sched == SCHED_SFF)
        fprintf(stderr, "warning: SFF not implemented yet; using FIFO consume\n");
    ...
    printf("wserver: port=%d threads=%d buffers=%d\n", port, threads, buffers);
